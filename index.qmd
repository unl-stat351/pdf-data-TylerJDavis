...
> These are text pdf's so we would have to use some sort of text extracting function. We then would have to figure out how to extract tabular data from pdf pages or text.
...
1.  Indentations in names across all departments
2.  Different columns for different people in the same department
    1.  Such as some people have multiple columns and some do not
3.  New categories in some files that are not in others

Look at college of arts and sciences for screenshots
...
Detect columns boundaries and then use that to read it in table wise. I also need to figure out a way to extract tables in a format that I can actually do something with.
...
library(pdftools)
library(stringr)
files <- list.files()[15:length(list.files())]

pdf <- function(x){
text <- pdf_info(x)$metadata
}


library(purrr)

info <- pdf_info(files[1])
...
git rm -r --cached rjavaenv
git commit -m "Remove large rjavaenv folder"
...
library(pdftools)
pdf <- pdf_text(x) 
pdf_df <- data.frame(Page = seq_along(pdf), Text = pdf, stringsAsFactors = FALSE)
return(pdf_df)
...
tables <- extract_tables(files[4], page = 500:530, output = "tibble")
```
